...
-- === НОВАЯ ФУНКЦИЯ ДЛЯ TSIS 1 (ДОБАВЛЕНО) ===

-- Поиск по всем полям (включая email и дополнительные телефоны)
CREATE OR REPLACE FUNCTION search_contacts(p_query TEXT)
RETURNS TABLE(contact_id INT, first_name VARCHAR, email VARCHAR, phone VARCHAR, phone_type VARCHAR) AS $$
BEGIN
    RETURN QUERY 
    SELECT c.id, c.first_name, c.email, p.phone, p.type
    FROM phonebook c
